...
.method public static f(Ljava/lang/String;)Ljava/lang/String;
    ...
    const-string v1, "(async function(k){try{if(!k){console.warn('Set Google AI API key in Settings > Translate first.');return;}const W=window,D=document,ID='kiwi-ai-translate';if(W.__kiwiAiTranslator&&W.__kiwiAiTranslator.version>=7){W.__kiwiAiTranslator.key=k;W.__kiwiAiTranslator.show(false);if(W.__kiwiAiTranslator.blockMedia)W.__kiwiAiTranslator.blockMedia();return;}const LANGS={'zh-CN':'中文','en':'English','ru':'Русский','uk':'Українська'};const S={version:7,key:k,target:localStorage.getItem('kiwi_ai_translate_target')||'zh-CN',enabled:false,busy:false,queue:[],seen:new WeakMap(),cache:new Map(),next:1,timer:0,live:/douyin|iesdouyin|kuaishou|gifshow|kwai/i.test(location.hostname),translated:0,failed:0,mediaPatched:false,mediaTimer:0,mediaObserver:null,observer:null};if(!LANGS[S.target])S.target='zh-CN';const UI={};function el(t,a,txt){const x=D.createElement(t);if(a)for(const k in a)x.setAttribute(k,a[k]);if(txt!=null)x.textContent=txt;return x;}function ensureUi(){if(UI.panel)return;const st=el('style');st.textContent='#'+ID+'-panel{position:fixed!important;left:50%!important;top:12px!important;transform:translateX(-50%)!important;z-index:2147483647!important;display:flex!important;gap:8px!important;align-items:center!important;max-width:calc(100vw - 24px)!important;padding:8px 10px!important;border-radius:8px!important;background:rgba(255,255,255,.97)!important;color:#202124!important;font:13px Arial,sans-serif!important;box-shadow:0 6px 24px rgba(0,0,0,.22)!important}#'+ID+'-panel button,#'+ID+'-panel select{font:13px Arial,sans-serif!important;border:1px solid #dadce0!important;border-radius:6px!important;background:#fff!important;color:#202124!important;height:30px!important;padding:0 8px!important}#'+ID+'-panel button[data-primary=\"1\"]{background:#1a73e8!important;color:#fff!important;border-color:#1a73e8!important}#'+ID+'-status{white-space:nowrap!important;max-width:42vw!important;overflow:hidden!important;text-overflow:ellipsis!important}#'+ID+'-btn{display:none!important}';D.documentElement.appendChild(st);UI.panel=el('div',{id:ID+'-panel'});UI.detect=el('span',null,'检测: '+detectLang());UI.sel=el('select');Object.keys(LANGS).forEach(c=>{const o=el('option',{value:c},LANGS[c]);if(c===S.target)o.selected=true;UI.sel.appendChild(o);});UI.run=el('button',{'data-primary':'1'},'翻译');UI.stop=el('button',null,'停止');UI.close=el('button',null,'关闭');UI.status=el('span',{id:ID+'-status'},'待翻译');UI.panel.append(UI.detect,el('span',null,'目标'),UI.sel,UI.run,UI.stop,UI.close,UI.status);D.documentElement.appendChild(UI.panel);UI.run.onclick=run;UI.stop.onclick=stop;UI.close.onclick=dismiss;UI.sel.onchange=()=>{S.target=UI.sel.value;localStorage.setItem('kiwi_ai_translate_target',S.target);setStatus('目标: '+LANGS[S.target]);};}"

    invoke-virtual {v0, v1}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    const-string v1, "function detectLang(){let l=(D.documentElement.getAttribute('lang')||'').trim();if(l)return l;const s=(D.body&&D.body.innerText||'').slice(0,1200);if(/[\\u4e00-\\u9fff]/.test(s))return 'zh';if(/[іїєґІЇЄҐ]/.test(s))return 'uk';if(/[\\u0400-\\u04ff]/.test(s))return 'ru';if(/[A-Za-z\\u00c0-\\u024f]/.test(s))return 'en';return 'auto';}function refreshDetect(){if(UI.detect)UI.detect.textContent='检测: '+detectLang();}function dismissed(){try{return sessionStorage.getItem('kiwi_ai_translate_dismissed')==='1';}catch(_){return false;}}function dismiss(){try{sessionStorage.setItem('kiwi_ai_translate_dismissed','1');}catch(_){}ensureUi();UI.panel.style.display='none';}function show(force){ensureUi();if(force){try{sessionStorage.removeItem('kiwi_ai_translate_dismissed');}catch(_){}}if(!force&&dismissed()){UI.panel.style.display='none';return;}UI.panel.style.display='flex';refreshDetect();}function setStatus(t){ensureUi();UI.status.textContent=t||('已译 '+S.translated+(S.queue.length?' / 队列 '+S.queue.length:''));}function stop(){S.enabled=false;S.queue.length=0;setStatus('已停止');}function run(){show(true);refreshDetect();S.enabled=true;S.translated=0;S.failed=0;blockMedia();observe();scan(D.body||D);schedule();}function nameOf(e){return ((e&&e.className&&String(e.className))+' '+(e&&e.id||'')+' '+Array.from(e&&e.attributes||[]).map(a=>a.name+' '+a.value).join(' ')).toLowerCase();}function isDanmu(e){for(let p=e;p&&p!==D.body;p=p.parentElement){if(/danmu|danmaku|barrage|bullet|webcast.+screen|screen.+comment/.test(nameOf(p)))return true;}return false;}function r(e){try{return e.getBoundingClientRect();}catch(_){return {left:0,top:0,right:0,bottom:0,width:0,height:0};}}function visible(e){const x=r(e),s=getComputedStyle(e);return x.width>0&&x.height>0&&s.display!=='none'&&s.visibility!=='hidden'&&s.opacity!=='0';}"

    invoke-virtual {v0, v1}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    const-string v1, "function nearViewport(e){if(!e)return true;const x=r(e),pad=Math.max(320,innerHeight*.9);return x.bottom>=-pad&&x.top<=innerHeight+pad&&x.right>=-80&&x.left<=innerWidth+80;}function inChat(e){if(!e||isDanmu(e))return false;for(let p=e;p&&p!==D.body;p=p.parentElement){const n=nameOf(p);if(/comment|chat|message|webcast|msg|room/.test(n))return true;}const x=r(e);return S.live&&x.left>innerWidth*.45&&x.width<innerWidth*.6&&x.height<180;}function badParent(p){if(!p)return true;const tag=p.tagName;if(/^(SCRIPT|STYLE|TEXTAREA|INPUT|NOSCRIPT|CODE|PRE|SVG|CANVAS|SELECT|OPTION|BUTTON)$/.test(tag))return true;if(isDanmu(p))return true;if(p.isContentEditable||p.closest('[aria-hidden=\"true\"],[data-kiwi-live-noise=\"1\"],#'+ID+'-panel,#'+ID+'-btn'))return true;if(S.live&&!inChat(p))return true;return !visible(p);}function nickname(e){const n=nameOf(e);return /nick|nickname|username|user-name|author|avatar|badge|fans|level/.test(n)&&!/content|comment|message|chat|text/.test(n);}function looksName(s){return s.length<=64&&!/[。.!?？！]/.test(s);}function prevColon(e){for(let p=e;p&&p!==D.body;p=p.parentElement){let q=p.previousSibling,h=0;while(q&&h++<6){const txt=(q.innerText||q.textContent||'').replace(/\\s+/g,' ').trim();if(txt)return /[:：]\\s*$/.test(txt)&&looksName(txt.replace(/[:：]\\s*$/,''));q=q.previousSibling;}}return false;}function textPart(t,e){const raw=(t||'').replace(/\\s+/g,' ').trim();if(!raw||nickname(e))return null;if(/^(https?:|www\\.)/i.test(raw))return null;if(!/[A-Za-z\\u00c0-\\u024f\\u0400-\\u04ff\\u0590-\\u08ff\\u3400-\\u9fff]/.test(raw))return null;const chat=S.live||inChat(e);let m=raw.match(/^([^:：]{1,64})([:：]\\s*)(.+)$/);if(m&&looksName(m[1])&&m[3].trim())return {prefix:m[1]+m[2],input:m[3].trim(),raw:raw};m=raw.match(/^(.{1,64}?)(?:说|發言|发言)\\s*[:：]\\s*(.+)$/);if(m&&looksName(m[1])&&m[2].trim())return {prefix:m[1]+': ',input:m[2].trim(),raw:raw};if(chat){const c=raw.search(/[:：]/);if(c>=0){const name=raw.slice(0,c).trim(),msg=raw.slice(c+1).trim();if(looksName(name)&&msg)return {prefix:name+raw.charAt(c)+' ',input:msg,raw:raw};return null;}if(prevColon(e))return {prefix:'',input:raw,raw:raw};return null;}if(raw.length<2)return null;return {prefix:'',input:raw,raw:raw};}"

    invoke-virtual {v0, v1}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    const-string v1, "function accept(n){return n&&n.nodeType===3&&n.__kiwiAiDone!=='1'&&!badParent(n.parentElement)&&(!S.live||inChat(n.parentElement))&&nearViewport(n.parentElement)&&!!textPart(n.nodeValue,n.parentElement);}function scan(root){if(!root)return;if(root.nodeType===3){if(accept(root))enqueue(root);return;}if(root.nodeType!==1&&root.nodeType!==9&&root.nodeType!==11)return;const w=D.createTreeWalker(root,NodeFilter.SHOW_TEXT,{acceptNode:n=>accept(n)?NodeFilter.FILTER_ACCEPT:NodeFilter.FILTER_REJECT});let n;while((n=w.nextNode()))enqueue(n);}function enqueue(n){const p=textPart(n.nodeValue,n.parentElement);if(!p)return;const sig=S.target+'\\n'+p.input;if(S.seen.get(n)===sig)return;S.seen.set(n,sig);if(S.cache.has(sig)){apply(n,p,S.cache.get(sig));return;}S.queue.push({id:S.next++,node:n,part:p,key:sig});}function schedule(){if(!S.enabled)return;clearTimeout(S.timer);S.timer=setTimeout(process,80);}async function process(){if(S.busy||!S.enabled)return;S.busy=true;try{while(S.enabled&&S.queue.length){const it=S.queue.shift();if(!it||!it.node.parentNode)continue;setStatus('翻译当前屏幕...');await callOne(it);}}finally{S.busy=false;setStatus();if(S.enabled&&S.queue.length)schedule();}}function cleanOut(t){return String(t||'').trim().replace(/^```[a-z]*\\s*/i,'').replace(/```$/,'').trim().replace(/^[\"“”']+|[\"“”']+$/g,'').trim();}async function callOne(it){const target=LANGS[S.target]||S.target;const system='你是全球直播间弹幕极速翻译引擎,自动识别输入的独立弹幕文本,进行纠错与本地化处理,并流利地翻译成目标语言。昵称在冒号前，不要翻译昵称，只翻译冒号后的发言。只输出译文,严禁解释、双语对照或注释。 无法翻译时则不输出任何文字。';const body={systemInstruction:{parts:[{text:system}]},contents:[{role:'user',parts:[{text:'目标语言: '+target+' ('+S.target+').\\n'+it.part.input}]}],generationConfig:{temperature:0,candidateCount:1}};const models=['gemini-3.5-flash','gemini-3.1-flash-lite'];let last;"

    invoke-virtual {v0, v1}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    const-string v1, "for(const m of models){try{const res=await fetch('https://generativelanguage.googleapis.com/v1beta/models/'+m+':generateContent?key='+encodeURIComponent(S.key),{method:'POST',mode:'cors',credentials:'omit',cache:'no-store',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});const j=await res.json();if(!res.ok)throw new Error((j.error&&j.error.message)||('HTTP '+res.status));const text=(j.candidates&&j.candidates[0]&&j.candidates[0].content&&j.candidates[0].content.parts||[]).map(p=>p.text||'').join('');const out=cleanOut(text);if(out){S.cache.set(it.key,out);apply(it.node,it.part,out);}return;}catch(e){last=e;}}S.failed++;console.warn('Google AI translation failed: '+(last&&last.message?last.message:last));}function apply(n,p,v){if(!n.parentNode)return;const out=cleanOut(v);if(!out)return;n.__kiwiAiDone='1';n.nodeValue=(p.prefix||'')+out;const e=n.parentElement;if(e){e.style.setProperty('color','#fff','important');e.style.setProperty('-webkit-text-fill-color','#fff','important');e.style.setProperty('text-shadow','0 1px 2px rgba(0,0,0,.85)','important');}S.seen.set(n,S.target+'\\n'+out);S.translated++;}function observe(){if(S.observer)return;S.observer=new MutationObserver(ms=>{blockMedia();if(!S.enabled)return;for(const m of ms){if(m.type==='characterData')scan(m.target);else m.addedNodes&&m.addedNodes.forEach(scan);}schedule();});S.observer.observe(D.documentElement,{childList:true,subtree:true,characterData:true});}"

    invoke-virtual {v0, v1}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    const-string v1, "function mediaUrl(u){u=String(u||'');let h='';try{h=new URL(u,location.href).hostname;}catch(_){}return /(\\.m3u8|\\.mp4|\\.flv|\\.m4s|\\.m4v|\\.webm|\\.mpd|\\.ts(?:[?#]|$)|videoplayback|\\/video\\/|\\/stream\\/|pull-flv|pull-hls|live-play|playurl|stream_addr)/i.test(u)||/(^|\\.)(douyinvod|douyincdn|douyinstatic|amemv|volcvod|bytecdn|byteimg|pstatp|kwaicdn|kwimgs|ksyuncdn|yximgs|gifshow)\\.(com|cn|net)$/i.test(h);}function hideNode(e){try{e.setAttribute('data-kiwi-live-noise','1');e.style.setProperty('display','none','important');e.style.setProperty('visibility','hidden','important');}catch(_){}}function stopMediaNode(v){try{if(v.pause)v.pause();v.muted=true;v.autoplay=false;v.preload='none';v.removeAttribute('autoplay');v.removeAttribute('src');if('srcObject'in v)v.srcObject=null;v.querySelectorAll&&v.querySelectorAll('source,track').forEach(s=>{s.removeAttribute('src');s.removeAttribute('srcset');});if(v.load)v.load();}catch(_){}hideNode(v);}function patchMediaApis(){if(S.mediaPatched)return;S.mediaPatched=true;try{const play=HTMLMediaElement.prototype.play;HTMLMediaElement.prototype.play=function(){stopMediaNode(this);return Promise.resolve();};const setAttr=Element.prototype.setAttribute;Element.prototype.setAttribute=function(n,val){const tag=(this.tagName||'').toUpperCase();if(/^(VIDEO|AUDIO|SOURCE|TRACK)$/.test(tag)&&/^(src|srcset|poster|autoplay)$/i.test(n))return;return setAttr.apply(this,arguments);};const fetch0=W.fetch;if(fetch0)W.fetch=function(input,init){const u=typeof input==='string'?input:(input&&input.url)||'';if(mediaUrl(u))return Promise.reject(new TypeError('video blocked'));return fetch0.apply(this,arguments);};const open=XMLHttpRequest.prototype.open;XMLHttpRequest.prototype.open=function(m,u){this.__kiwiBlockMedia=mediaUrl(u);return open.apply(this,arguments);};const send=XMLHttpRequest.prototype.send;XMLHttpRequest.prototype.send=function(){if(this.__kiwiBlockMedia){try{this.abort();}catch(_){}return;}return send.apply(this,arguments);};}catch(_){}}"

    invoke-virtual {v0, v1}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    const-string v1, "function blockMedia(){if(!S.live)return;patchMediaApis();let st=D.getElementById(ID+'-media-block');if(!st){st=el('style',{id:ID+'-media-block'});st.textContent='video,audio{display:none!important;visibility:hidden!important;pointer-events:none!important}';D.documentElement.appendChild(st);}D.querySelectorAll('video,audio').forEach(stopMediaNode);D.querySelectorAll('source,track').forEach(s=>{if(s.closest('video,audio')){s.removeAttribute('src');s.removeAttribute('srcset');hideNode(s);}});D.querySelectorAll('canvas').forEach(c=>{const x=r(c);if(x.width>80&&x.height>80&&x.left<innerWidth*.82)hideNode(c);});if(!S.mediaObserver){S.mediaObserver=new MutationObserver(()=>blockMedia());S.mediaObserver.observe(D.documentElement,{childList:true,subtree:true,attributes:true,attributeFilter:['src','srcset','autoplay']});}if(!S.mediaTimer)S.mediaTimer=setInterval(blockMedia,1200);}W.__kiwiAiTranslator={version:7,key:k,show:show,run:run,stop:stop,scan:scan,dismiss:dismiss,blockMedia:blockMedia};ensureUi();show(false);blockMedia();}catch(e){console.warn('Google AI translation failed: '+(e&&e.message?e.message:e));}})("
